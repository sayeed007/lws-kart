import { Body, Head, Html, Img, Link, Tailwind } from '@react-email/components';
import moment from 'moment';
import { NumberToWord } from '@/utils/NumberToWord';
import LWSLogo from '../../../public/assets/images/logo.svg';

const InvoiceForEmail = ({ userName, userOrders, userAddress, invoiceLink }) => {
                    ...
                    <div className="w-[90%] mx-auto">
                        ...
                        <div className="flex w-full justify-between items-center">
                            ...
                            <div className="flex flex-col">
                                <div className="py-2">
                                    Invoice:<span className="font-bold ml-2">{userOrders?.id}</span>
                                </div>
                                <br />
                                <div className="py-2">
                                    Order Time:<span className="font-bold uppercase ml-2"> {moment(userOrders?.orderTime).format('DD MMM, YYYY')}</span>
                                </div>
                            </div>
                                    ...
                                    >
                                        <table className="min-w-full divide-y divide-gray-200">
                                            ...
                                            <tbody className="bg-white divide-y divide-gray-200">
                                                {/* Table rows */}
                                                {(userOrders?.orderDetailsId)?.map((item, index) => (
                                        ...
                                        <div className="flex w-full justify-between items-center mt-16 min-h-[100px] bg-gray-100 p-3">
                                            ...
                                            <div>
                                                Total in words: <span className="ml-6 font-bold">
                                                    {NumberToWord(userOrders?.totalPrice ? Number(userOrders?.totalPrice) : 0)}
                                                </span>
                                            </div>

                                            {/* TOTAL */}
                                            <div className="mr-16">
                                                Total: <span className="ml-6 font-bold">{userOrders?.totalPrice}</span>
                                            </div>
                                            ...
                                            if (key !== '_id') {
                                                return (
                                                    <p
                                                        ...
                                                        key={key}
                                                    >
                                                        {key}:  <span className='font-bold'>{value ? value : 'N/A'}</span>
                                                    </p>
                                                )
                                            }
                                        })
                        ...
                        <div className='flex'>
                            ...
                            <Link
                                className="text-primary hover:text-blue-700 font-bold cursor-pointer mx-3"
                                href={invoiceLink}>
                                View Invoice
                            </Link>
                        </div>
